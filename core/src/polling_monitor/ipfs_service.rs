...
use graph::{cheap_clone::CheapClone, ipfs_client::IpfsClient, tokio::sync::Semaphore};
...
impl IpfsService {
    ...
    async fn call(&self, cid: Cid) -> Result<Option<Bytes>, Error> {
        ...
        let stat = match self.client.object_stat(&cid_str, self.timeout).await {
            Ok(stat) => stat,
        ...
        if stat.cumulative_size > self.max_file_size {
            return Err(anyhow!(
