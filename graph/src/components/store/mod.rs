...
impl StableHash for EntityKey {
    fn stable_hash<H: stable_hash::StableHasher>(&self, field_address: H::Addr, state: &mut H) {
        self.subgraph_id.stable_hash(field_address.child(0), state);

        stable_hash::StableHash::stable_hash(
            &self.entity_type.as_str(),
            field_address.child(1),
            state,
        );
        stable_hash::StableHash::stable_hash(&self.entity_id, field_address.child(2), state);
    }
}
